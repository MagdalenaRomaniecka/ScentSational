...
# --- 3. DATA LOADING (FIX FOR .XSL EXTENSION) ---
@st.cache_data
def load_data():
    try:
        # Próba 1: Wczytaj jako CSV (najbardziej prawdopodobne, że to CSV ze zmienioną nazwą)
        try:
            df = pd.read_csv("perfumes_dataset.xsl")
        except:
            # Próba 2: Jeśli to jednak Excel, spróbuj silnika Excela
            try:
                df = pd.read_excel("perfumes_dataset.xsl")
            except:
                return None, None
    ...
    with st.container():
        ...
        with col1:
            if img_url:
                ...
                st.markdown(f"<div style='width:80px; height:80px; border:1px solid #D4AF37; border-radius:50%; display:flex; align-items:center; justify-content:center; color:#D4AF37; margin:0 auto;'>{initials}</div>", unsafe_allow_html=True)
...
if df is not None:
    ...
    st.error("CRITICAL: 'perfumes_dataset.xsl' not found or invalid format.")